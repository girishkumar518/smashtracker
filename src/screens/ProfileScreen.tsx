                ...
                userClubs.filter(c => !isPersonalClubId(c.id)).map(club => (
                  ...
                  >
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      ...
                      <View style={{ marginLeft: 10 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                          ...
                          {user?.defaultClubId === club.id && <Text style={{ color: '#ED8936', fontSize: 10, fontWeight: 'bold' }}>DEFAULT</Text>}
                        </View>
                      </View>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                      ...
                      <TouchableOpacity
                        onPress={(e) => {
                          e.stopPropagation(); // Don't switch club, just set default
                          const isDefault = user?.defaultClubId === club.id;
                          if (isDefault) {
                            // Toggle off? Maybe, or just keep it on. Let's allow toggle off.
                            updateProfile({ defaultClubId: '' });
                          } else {
                            updateProfile({ defaultClubId: club.id });
                          }
                        }}
                        hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                      >
                        <Ionicons
                          name={user?.defaultClubId === club.id ? "star" : "star-outline"}
                          size={22}
                          color={user?.defaultClubId === club.id ? "#ED8936" : theme.colors.textSecondary}
                        />
                      </TouchableOpacity>
